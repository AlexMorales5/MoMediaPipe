...
while cap.isOpened():
    ...
    button_position = (int(cap.get(cv2.CAP_PROP_FRAME_WIDTH)) - 200, int(cap.get(cv2.CAP_PROP_FRAME_HEIGHT)) - 70, int(cap.get(cv2.CAP_PROP_FRAME_WIDTH)) - 20, int(cap.get(cv2.CAP_PROP_FRAME_HEIGHT)) - 20)
    cv2.rectangle(frame, (button_position[0], button_position[1]), (button_position[2], button_position[3]), (48, 213, 200), cv2.FILLED)  # Turquoise color
    add_text(frame, "Click to refresh", (int(cap.get(cv2.CAP_PROP_FRAME_WIDTH)) - 190, int(cap.get(cv2.CAP_PROP_FRAME_HEIGHT)) - 35), color=(255, 192, 203), font_scale=1, thickness=2)  # Smaller text
